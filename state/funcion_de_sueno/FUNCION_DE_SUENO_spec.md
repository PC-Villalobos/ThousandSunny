...
Version: 0.2 (contrato comun, dos perfiles)
Estado: activo
...
para consolidar lo vivido, auditar coherencia, detectar deriva, medir repeticion
de asignaciones declaradas y dejar un parte legible al despertar.
...
> **Referencia de codigo.** Este directorio (`state/funcion_de_sueno/`) es la
> referencia versionada del motor. No es autoridad operacional: esa sigue en
> Hipatia Local / Bitacora JSONL. Hay dos perfiles con estado separado, declarados
> en `sleep_profiles.v1.json` y explicados en `CONTRATO_DOS_PERFILES_V1.md`.
> Cualquier doc del vault local
...
```json
{
  ...
  "timestamp": "ISO-8601",
  "scope_id": "thousandsunny-repo | groot-local-full-corpus",
  "executor": "github-actions | codex-local-cron | other",
  "actor": "deterministic-sleep-engine | other",
  "role": "Nami | Robin | Chopper | Vivi | Usopp | Zoro | Sanji | Jimbe | Franky | Groot",
  ...
  "guardrails": {
    ...
    "rotation_decision": "human_required",
    "role_repetition_day_threshold": 3
  }
}
```
...
| REM  | Integracion   | Repeticion por ejecucion/dia, candidato de rol, aprendizaje |
...
- `sleep_profiles.v1.json` - identidades y fronteras de los dos perfiles.
- `sleep_ledger.jsonl` - una linea por corrida del perfil repo; no se fusiona con el local.
- `sleep_state.json` - estado de la ultima corrida (lo escribe el skill).
- `reports/SLEEP_<fecha>.md` - partes generados.

## Configuracion de la costura operacional

| Variable | Uso |
|---|---|
| `HIPATIA_BITACORA_URL` | Endpoint local de Hipatia; por defecto `http://127.0.0.1:8765` |

No contiene credenciales. Si Hipatia no es alcanzable, el adaptador informa
degradacion y no sustituye la autoridad por GAS ni por el ledger del repo.
...
evidencia, revision humana ante repeticion por dias, v1 aspiracional, respetar RETOMAR.
