...
5. **Environment:** *Default* (red **Trusted**) basta para leer/escribir el repo
   y abrir el PR. Si quieres POSTear a la Bitacora GAS de PuenteDeMando, edita el
   environment -> Network access -> **Custom** y permite el dominio del web app
   de GAS (`script.google.com`, `*.googleusercontent.com`). Si no, el bloque de
   bitacora queda en el parte como respaldo.
...
4. Salida:
   ...
   - Si hay deriva significativa, incluye en el parte el bloque de Bitacora GAS
     (nakama=Usopp, tema=caso0, mensaje=...).
