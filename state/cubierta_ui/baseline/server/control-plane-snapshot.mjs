const COPY = {
  ...
  outcome: {
    ...
    unknown: ['Resultado histórico desconocido', 'No se reconstruye hacia atrás lo que el registro no capturó.'],
  },
  epistemic: {
    ...
    unknown: ['Desconocido', 'El registro no permite asignar un estatuto más preciso.'],
  },
  execution: {
  ...
  return {
    ...
    agents: order.targets.map(target => {
      ...
      return {
        ...
        outcome: explain('outcome', delivery.deliberation_outcome || 'unknown'),
        epistemic: explain('epistemic', delivery.epistemic_status || 'unknown'),
