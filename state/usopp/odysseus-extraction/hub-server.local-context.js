...
function authMiddleware(req, res, next) {
  ...
  const pub = ["/", "/puente", "/manifest.json", "/sw.js", "/api/status"];
  if (pub.includes(req.path) || req.path.startsWith("/icon")) return next();
  // Verificar token Bearer o query param
  const bearer = (req.headers.authorization || "").replace("Bearer ", "");
  const qToken = req.query.token || req.body?.token;
  if (bearer === HUB_TOKEN || qToken === HUB_TOKEN) return next();
  res.status(401).json({ error: "Token invalido. Anade Authorization: Bearer <HUB_TOKEN>" });
}
...
app.post("/api/codex", async (req, res) => {
  ...
  try {
    const projectIds = inferProjectIds(tarea, workingDir || "");
    rememberCaptainTurn(tarea, req.path, "api/codex", projectIds, ["hub", "captain-turn", "codex"]);
    const ananda = await captureAnanda({
      text        : tarea,
      route       : req.path,
      source      : "api/codex",
      context     : workingDir ? `workingDir: ${workingDir}` : "",
      responses   : ["Usopp"],
      logFn       : logBitacora,
      broadcastFn : broadcast,
    });
    const r = await callCodex(tarea, { workingDir });
